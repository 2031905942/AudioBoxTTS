...
class IndexTTS2:
    def __init__(
        ...
        # 修改：从本地加载 w2v-bert-2.0
        w2v_bert_path = os.path.join(self.model_dir, "facebook/w2v-bert-2.0")
        if os.path.exists(w2v_bert_path):
            print(f">> Loading w2v-bert-2.0 from local: {w2v_bert_path}")
            self.extract_features = SeamlessM4TFeatureExtractor.from_pretrained(w2v_bert_path)
        else:
            print(">> Loading w2v-bert-2.0 from HuggingFace (facebook/w2v-bert-2.0)")
        ...
        # 修改：从本地加载 MaskGCT semantic_codec
        maskgct_path = os.path.join(self.model_dir, "amphion/MaskGCT/semantic_codec/model.safetensors")
        if os.path.exists(maskgct_path):
            print(f">> Loading MaskGCT semantic_codec from local: {maskgct_path}")
            semantic_code_ckpt = maskgct_path
        else:
            print(">> Loading MaskGCT semantic_codec from HuggingFace (amphion/MaskGCT)")
