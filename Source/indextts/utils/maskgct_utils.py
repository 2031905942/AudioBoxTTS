...
def build_semantic_model(path_='./models/tts/maskgct/ckpt/wav2vec2bert_stats.pt', model_dir=None):
    # 尝试从本地加载
    if model_dir:
        local_path = os.path.join(model_dir, "facebook/w2v-bert-2.0")
        if os.path.exists(local_path):
            print(f">> Loading w2v-bert-2.0 model from local: {local_path}")
            semantic_model = Wav2Vec2BertModel.from_pretrained(local_path)
        else:
